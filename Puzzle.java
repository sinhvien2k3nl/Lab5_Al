...
public class Puzzle {
	...
	public int computeH2(Node currentState) {
		...
		for(int i = 0; i< MAX_ROW;i++) {
			for(int j = 0; j< MAX_COL;j++) {
				int[] currentPos = currentState.getLocation(currentState.getTile(i, j));
				int[] goalPos = goalState.getLocation(goalState.getTile(i, j));
				re +=PuzzleUtils.manhattanDistance(currentPos, goalPos);
			}
		}
